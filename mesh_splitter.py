...
def ensure_volume(mesh):
    """Saneado interno ligero para que las booleanas manifold acepten una
    PIEZA (exigen volúmenes cerrados): arreglos de trimesh + soldado manifold.
    No es un reparador de modelos del usuario — los STL rotos se detectan al
    cargar y se recomienda repararlos en una herramienta externa."""
...
class MeshSplitterApp(QMainWindow):
    ...
    def _load_path(self, path):
        ...
        # Reparar mallas NO es competencia de esta app (decisión de alcance,
        # 11/06/2026): se detecta, se avisa claro, y se recomienda herramienta
        # externa. El usuario decide si continúa.
        repair_note = ""
        if not mesh.is_volume:
            repair_note = ("\nAVISO: malla con defectos (no watertight). "
                           "Recomendado repararla en una herramienta externa.")
            QMessageBox.warning(
                self, "Malla con defectos",
                "Este STL no es un sólido cerrado (watertight).\n\n"
                "Puedes continuar y cortar igualmente, pero la fusión de piezas "
                "pequeñas y las espigas pueden fallar donde la geometría esté rota.\n\n"
                "Recomendado: repara el modelo primero en una herramienta externa "
                "y carga el archivo reparado.\n"
                "  •  Bambu Studio / OrcaSlicer: importar → reparar → exportar STL\n"
                "  •  Meshmixer, Blender, Windows 3D Builder…")
